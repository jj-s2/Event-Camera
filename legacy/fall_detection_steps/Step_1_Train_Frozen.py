# -*- coding: utf-8 -*-
"""
Step 1 Frozen: 冻结 backbone，只训练分类头
适合数据量少或迁移学习场景
"""
...
def freeze_backbone(model):
    """冻结 RGB 和 Event 的 backbone，只训练 classifier"""
    for param in model.rgb_features.parameters():
        param.requires_grad = False
    for param in model.ev_features.parameters():
        param.requires_grad = False
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total     = sum(p.numel() for p in model.parameters())
    print(f"冻结 backbone：可训练参数 {trainable/1e6:.2f}M / 总 {total/1e6:.2f}M")
    ...
    for epoch in range(1, CFG["epochs"] + 1):
        ...
        if metrics["f1"] >= best_f1:
            ...
            torch.save({"epoch": epoch, "model_state": model.state_dict(),
                       ...
                       save_dir / "best_frozen.pth")
            print(f"  ✓ Saved best_frozen.pth  F1={best_f1:.4f}")

    with open(save_dir / "history_frozen.json", "w") as f:
        json.dump(history, f, indent=2)
    print(f"\n冻结训练完成，最优 F1={best_f1:.4f}")
